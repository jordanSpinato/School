...
//---------------------------------------------------------------
// Purpose of Function: mark the vertex with the corresponding visit number
// Purpose of parameter: take in the incrememted visit number and the vertex to add it to in the attribute defined for the class
// algorithm: as long as the number is less than the total count used for all vertex. if loop through that many times and if there parameter matches the vertex name assign the visit number to that vertex.. display the vertex name and updated visit number
// Return: void
void dgraph :: visit ( int visitNum, char vTex)
{
    ...
    for (int i = 0; i < countUsed; i++)
    {
        if ( Gtable[i].vertexName == vTex)
        {
            Gtable[i].visit = visitNum;
            cout << "Visit number for Vertx '" << vTex << "' has been updated to: " << visitNum << endl;
            return;
        }

    } // end for

    cout << " Vertex not found: " << vTex << endl;
...
//---------------------------------------------------------------
// Purpose of Function: check to see if the current vertex has been marked
// Purpose of parameter: take the character of the given vertex
// algorithm: loop through as many vertex we have. if there is a match with the parameter and the vertex name return the visit number. if it has not been visited it will return 0 which is bool false. any other visit number will return true
// Return: 0 for no visit ... > 1 for a visit
bool dgraph :: isMarked (char vTex)
{
	// loop for how many vertex we have
    for (int i = 0; i < countUsed; i++)
    {
	// any number other than 0 returns true
        if (Gtable[i].vertexName == vTex)
            return Gtable[i].visit;
